...
const Game:React.FC<IGameProps> = ({solution}) => {
    ...
    const {currentGuess,handleKeyUp,handleKeyboardEvent,guesses,trial,isCorrect,usedKeys,NUM_TRIALS,error,clearError} = useMathler(solution,state.difficulty.squaresCount);
    useEffect(() => {
    ...
    return (
        <div className={style.game}>
            ...
            <Modal isOpen={showModal} toggleModal={handleToggleModal}>
                ...
                <div className = {style.endgame__grid} id="share">
                    {
                        guesses.map((guess,i) => (
                            <div key={i} className={style.row}>
                                {
                                    guess ? guess.map((char,i) => (
                                        <span key={i}>{SQUARES[char.color]}</span>
                                    )) : ""
                                }
                            </div>
                        ))
                ...
                <button className={style.copy__grid} onClick={copyToClipboard}>Copy to Clipboard</button>
            </Modal>
            {error && <Toast isOpen={true} background={error.color}>{error.message}</Toast>}
        </div>
    )
}
